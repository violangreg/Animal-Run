...
*/
	...
	void OnTriggerEnter2D(Collider2D collider)
	{
        ...
        {
            float widthOfBGObject = collider.bounds.size.x;		// get the width of the collided object
            Vector3 position = collider.transform.position;		// get the position of the collided object

			position.x += widthOfBGObject * numPanels;			// multiple the width and the number of objects of that object (illusion of forever scroller)
